...
function useStatusPresentation(status: Status) {
  ...
  switch (status.kind) {
    case "formatting":
      return {
        label: t("status.formatting"),
        className: "is-running",
        icon: <LoaderCircle className="spin" size={17} />,
      };
    case "changed":
      return {
    ...
    case "unchanged":
      return {
        label: t("status.unchanged"),
        className: "is-neutral",
        icon: <CircleDot size={17} />,
      };
    case "failure":
      return {
        label: t("status.fail-closed"),
    ...
    case "error":
      return {
        label: t("status.unexpected-error"),
        className: "is-failure",
        icon: <AlertOctagon size={17} />,
      };
    default:
...
export function FormatStatus({
  ...
  return (
    <div className="format-status" role="status" aria-live="polite">
      <span className={`status-kind ${presentation.className}`}>
        {presentation.icon}
        {presentation.label}
      </span>
      {status.kind === "outdated" ? (
        <span className="status-detail">{t("status.outdated.detail")}</span>
      ) : null}
      {duration !== undefined ? (
        <span>
          <Clock3 size={15} aria-hidden="true" /> {duration.toFixed(1)} ms
        </span>
      ) : null}
      <span className="status-profile">
        {t("status.profile")}:{" "}
        <strong>{profileKey ? t(profileKey) : profile}</strong>
      </span>
      <span className="status-versions">
        {t("status.versions", { web: webVersion, fmt: formatterVersion })}
      </span>
    </div>
  );
}
