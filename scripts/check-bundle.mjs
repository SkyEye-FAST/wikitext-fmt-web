...
const appGraph = JSON.parse(
  await readFile(new URL("app-bundle-graph.json", distDirectory), "utf8"),
);
const workerGraph = JSON.parse(
  await readFile(new URL("formatter-worker-graph.json", distDirectory), "utf8"),
);
...
if (!headers.includes(rootNoTransformRule)) {
  throw new Error(
    "The production root must opt out of Cloudflare response transformation.",
  );
}
...
function findManifestEntry(sourceSuffix) {
  const matching = Object.entries(manifest).filter(([key]) =>
    key.endsWith(sourceSuffix),
  );
  if (matching.length !== 1) {
    throw new Error(
      `Expected one Vite manifest entry ending in ${sourceSuffix}; found ${matching.length}.`,
    );
  }
  return matching[0];
}
...
function collectChunkGraph(graph, entry, includeDynamicImports) {
  const chunksByFile = new Map(
    graph.chunks.map((chunk) => [chunk.file, chunk]),
  );
  ...
  const visit = (file) => {
    ...
    if (!chunk)
      throw new Error(`Bundle graph references missing chunk ${file}.`);
...
if (!initialFile) {
  throw new Error(
    "Could not identify the initial application entry from the Vite manifest.",
  );
}
const initialChunk = appGraph.chunks.find(
  (chunk) => chunk.file === initialFile,
);
if (!initialChunk) {
  throw new Error(
    "The Vite initial application entry is absent from the emitted application graph.",
  );
}
const initialEntries = collectManifestEntries("index.html");
const initialChunks = collectChunkGraph(appGraph, initialChunk, false);
const initialCss = [
  ...new Set(initialEntries.flatMap((entry) => entry.css ?? [])),
];

const [, diffEntry] = findManifestEntry("/DiffView.tsx");
const [, settingsEntry] = findManifestEntry("/SettingsPanel.tsx");
const workerEntries = workerGraph.chunks.filter(
  (chunk) =>
    chunk.isEntry &&
    /(?:^|\/)src\/formatter\/formatter\.worker\.ts$/.test(
      chunk.facadeModuleId ?? "",
    ),
);
if (workerEntries.length !== 1) {
  throw new Error(
    `Expected one formatter Worker entry; found ${workerEntries.length}.`,
  );
}
...
for (const chunk of workerChunks) {
  for (const id of [
    ...
    if (builtinSpecifiers.has(id) || builtinSpecifiers.has(unprefixed)) {
      throw new Error(
        `Node built-in ${JSON.stringify(id)} found in the formatter Worker graph.`,
      );
    }
    for (const pattern of forbiddenModulePatterns) {
      if (pattern.test(id)) {
        throw new Error(
          `Forbidden module ${JSON.stringify(id)} found in the formatter Worker graph.`,
        );
      }
    }
  }
...
for (const id of initialModules) {
  if (
    /(?:^|\/)node_modules\/(?:\.pnpm\/[^/]+\/node_modules\/)?wikitext-fmt(?:\/|$)/.test(
      id,
    )
  ) {
    throw new Error(
      `Formatter runtime module leaked into the initial application graph: ${id}`,
    );
  }
}
...
for (const file of await sourceFiles(new URL("src/", projectDirectory))) {
  ...
  const deepImport =
    /(?:from\s+|import\s*\()\s*["']wikitext-fmt\/(?!browser["'])[^"']+/.exec(
      contents,
    );
  if (deepImport) {
    throw new Error(
      `Deep wikitext-fmt import found in ${path.relative(new URL(".", projectDirectory).pathname, file.pathname)}: ${deepImport[0]}`,
    );
  }
}
...
const reports = await Promise.all([
  ...
  sizeReport(
    "Formatter Worker graph",
    workerChunks.map((chunk) => chunk.file),
  ),
...
for (const report of reports) {
  console.log(
    `- ${report.label}: ${formatBytes(report.raw)} raw, ${formatBytes(report.gzip)} gzip`,
  );
...
console.log(
  "Manifest graph, Worker dependencies, and production headers: clear",
);
